...
int main()
{
    ...
    int i, j, n, b[10][10], sum = 0, sum1 = 0, a = 0, normal;
    ...
    for(i = 0; i < n; i++)   // to iterate the rows
    {
        ...
        {
            scanf("%d", &b[i][j]);
            a = b[i][j]*b[i][j];  // finding square of each element
            sum1 =sum1+ a;  // same as sum1 = sum1 + a
        }
    }
    ...
    for(i = 0; i < n; i++)
    {
        sum = sum + b[i][i];   // sum of the diagonal elements
    }
